...
function prepareAjaxObject(l, f) {
    ...
    if (f !== false) {
        ...
        $(f).find('input, select').each(function() {
            ...
            formElements[eid] = eva;
        });

      ad['formElements'] = formElements;
...
// if form submit as POST
    ajaxPost(ad, id);

// if link submit as GET
   // ajaxGet(ad, id);
}

function ajaxPost(ad, id) {
...
function ajaxSuccess(response, ad, id) {
  ...
  if (ad['ajaxSuccessAction'] !== undefined) {

    var sa = ad["ajaxSuccessAction"];
